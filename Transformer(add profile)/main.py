...
model = Transformer(args, args.peter_mask, src_len, tgt_len, src_vocab_size=ntokens, tgt_vocab_size=ntokens, d_model=args.emsize, n_layers=args.nlayers,n_heads=args.nhead,\
...
def train(data, epoch):
    ...
    while True:
        ...
        dec_inputs = seq[:,:-1].to(device)
        if args.use_feature:
            dec_inputs = torch.cat([feature,dec_inputs],dim=1)
        log_word_prob, rating_p, log_context_dis = model(user, item, enc_inputs, dec_inputs)  # (tgt_len, batch_size, ntoken) vs. (batch_size, ntoken) vs. (batch_size,)
...
def generate(data):
    ...
    with torch.no_grad():
        while True:
            ...
            token_flag = True
            enc_inputs = torch.cat([user_profile, item_profile], dim=1).to(device)
            dec_inputs = bos.t().to(device)


            if args.use_feature:
                dec_inputs = torch.cat([feature, dec_inputs], 1)
            # ids = dec_inputs.clone()
            start_idx = dec_inputs.size(1)
            for idx in range(args.words):
                # produce a word at each step
                if idx == 0:

                    log_word_prob, rating_p, log_context_dis = model(user, item, enc_inputs, dec_inputs, token_flag)  # (tgt_len, batch_size, ntoken) vs. (batch_size, ntoken) vs. (batch_size,)
                    rating_predict.extend(rating_p.tolist())
                    context = predict(log_context_dis, topk=args.words)  # (batch_size, words)
                    context_predict.extend(context.tolist())
                    tokens_predict = [ids2tokens(ids, word2idx, idx2word) for ids in context_predict]
                    # log_word_prob, log_context_dis, rating_p, _, _ = model(user, item, text, image_feature)  # (tgt_len, batch_size, ntoken) vs. (batch_size, ntoken) vs. (batch_size,)
  ...
  #                   context_predict.extend(context.tolist())
                    ...
                    log_word_prob, rating_p, log_context_dis = model(user, item, enc_inputs,dec_inputs,token_flag)
                ...
                dec_inputs = torch.cat([dec_inputs, word_idx.unsqueeze(1)],dim=1)  # (batch_size, seq_len)

                # dec_inputs = word_idx.unsqueeze(1)  # (len++, batch_size)
                # ids = torch.cat(dec_inputs) # (batch_size, seq_len)
            # ids = dec_inputs[start_idx:].t().tolist()
            idss_predict.extend(dec_inputs.tolist())
    ...
    tokens_predict = [ids2tokens(ids[1:], word2idx, idx2word) for ids in idss_predict]
...
# model_path = 'D:/PETER2/MT_output/rank_profile_Transfomer/AZ_MT_1/model.pt'
