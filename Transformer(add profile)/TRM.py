...
def get_attn_pad_mask(device, ui_len,seq_q, seq_k,pad_idx):
    ...
    return pad_attn_mask.expand(batch_size, len_q+ui_len, len_k+ui_len)  # batch_size x len_q x len_k
...
class Encoder(nn.Module):
    def __init__(self, src_emb,d_model,n_layers,n_heads, d_k, d_v,d_ff,dropout, pad_idx, nuser, nitem):
...
class Decoder(nn.Module):
    def __init__(self,tgt_emb, d_model,n_layers, n_heads, d_k, d_v, d_ff, dropout, pad_idx, peter_mask):
        ...
        self.ui_len = 2
    def forward(self, user_emb, item_emb, dec_inputs, enc_inputs, enc_outputs): # dec_inputs : [batch_size x target_len]
...
class Transformer(nn.Module):
    def __init__(self, args, peter_mask, src_len, tgt_len, src_vocab_size, tgt_vocab_size, d_model, n_layers, n_heads, d_k, d_v, d_ff, dropout, pad_idx, nuser, nitem):
        ...
        self.init_weights()
        self.encoder = Encoder(self.word_embeddings, d_model, n_layers, n_heads, d_k, d_v, d_ff, dropout, pad_idx,nuser, nitem)  ## 编码层
        self.decoder = Decoder(self.word_embeddings, d_model, n_layers, n_heads, d_k, d_v, d_ff, dropout, pad_idx, peter_mask)  ## 解码层
    ...
    def predict_seq(self, hidden):
        word_prob = self.projection(hidden[:,self.src_len:,:])  # (tgt_len, batch_size, ntoken)
        log_word_prob = func.log_softmax(word_prob, dim=-1)
        return log_word_prob


    def predict_context(self, hidden):
        context_prob = self.projection(hidden[:,1,:])  # (batch_size, ntoken)
        log_context_dis = func.log_softmax(context_prob, dim=-1)
        return log_context_dis

    def generate_token(self, hidden):
        word_prob = self.projection(hidden[:,-1,:])  # (batch_size, ntoken)
        log_word_prob = func.log_softmax(word_prob, dim=-1)
        return log_word_prob

    def forward(self, user, item, enc_inputs, dec_inputs,token_flag=False):
        ...
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(user_emb, item_emb, dec_inputs, enc_inputs, enc_outputs)
