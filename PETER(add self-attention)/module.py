...
class MLP_IMG(nn.Module):
    ...
    def forward(self, hidden):  # (batch_size, emsize)
        img_pred = self.sigmoid(self.linear1(hidden))  # (batch_size, emsize)
        # img_pred = torch.squeeze(self.linear2(mlp_vector))  # (batch_size,)
        return img_pred
...
def generate_img_mask(src_len, tgt_len):
    total_len = src_len + tgt_len
    mask = generate_square_subsequent_mask(total_len)
    mask[0, 1] = False
    mask[0, 2] = False
    mask[1, 2] = False# allow to attend for user and item
    return mask
...
class PETER(nn.Module):
    def __init__(self, args, peter_mask, src_len, tgt_len, pad_idx, nuser, nitem, ntoken, emsize, nhead, nhid, nlayers, dropout=0.5):
        super(PETER, self).__init__()
        self.pos_encoder_attention = PositionalEncoding(emsize, dropout)  # emsize: word embedding size
        self.pos_decoder = PositionalEncoding(emsize, dropout)  # emsize: word embedding size

        attention_layer = TransformerEncoderLayer(emsize, nhead, nhid, dropout)  # nhid: dim_feedforward, one basic layer, including multi-head attention and FFN
        self.attention_module = TransformerEncoder(attention_layer,nlayers)

        decoder_layers = TransformerDecoderLayer(emsize, nhead, nhid, dropout)  # nhid: dim_feedforward, one basic layer, including multi-head attention and FFN
        self.transformer_decoder = TransformerDecoder(decoder_layers, nlayers)  # loop over the one above
        ...
        self.emsize = emsize
        # if peter_mask and not self.image_fea:
        #     self.attn_mask = generate_peter_mask(src_len, tgt_len)
        # elif not peter_mask and self.image_fea:
        #     self.attn_mask = generate_img_mask(src_len, tgt_len)
        # else:
    ...
    def get_attn_pad_mask(self, seq_q, seq_k):
        len_q, batch_size = seq_q.size()
        len_k, batch_size = seq_k.size()
        # eq(zero) is PAD token
        pad_attn_mask = seq_k.data.eq(self.pad_idx).unsqueeze(1)  # batch_size x 1 x len_k, one is masking
    ...
    def predict_img(self, hidden):
        img_pred = self.img_reference(hidden[2])  # (batch_size,)
        return img_pred
    ...
    def forward(self, user, item, words, text, image_feature, seq_prediction=True, context_prediction=True, rating_prediction=True, attention = True):
        '''
        ...
        words_attn_mask = words.t() == self.pad_idx
        words_src = self.word_embeddings(words)
        words_src = words_src * math.sqrt(self.emsize)
        words_src = self.pos_encoder_attention(words_src)
        out, attns_weights = self.attention_module(words_src, mask= None,src_key_padding_mask = words_attn_mask)
        ...
        if self.image_fea:
            middle = torch.zeros(batch_size, 1).bool().to(device) #img
        ...
        if self.image_fea:
            img_src = image_feature.unsqueeze(0)
            src = torch.cat([u_src, i_src, img_src, w_src], 0)  # (total_len, batch_size, emsize)
        else:
        ...
        #attns: layer默认=2
        hidden, dec_self_attns, dec_enc_attns = self.transformer_decoder(src, out, attn_mask, key_padding_mask,words_attn_mask)  # (total_len, batch_size, emsize) vs. (nlayers, batch_size, total_len_tgt, total_len_src)
        if rating_prediction:
            rating = self.predict_rating(hidden)  # (batch_size,) 输入user的embedding
        else:
            rating = None
        ...
        if self.image_fea:
            img_fea_p = self.predict_img(hidden)  # (tgt_len, batch_size, ntoken)
        else:
            img_fea_p = None

        return log_word_prob, log_context_dis, rating, dec_self_attns, dec_enc_attns, img_fea_p
