...
parser.add_argument('--image_fea_path', type=str, default='D:/Pytorch-extract-feature/Res_embeddings/',
                    help='path for image_fea_path')
...
parser.add_argument('--item_keyword_path', type=str, default='./Amazon/Movies_and_TV/MT_keyword_one.pickle',
                    help='path for loading item_keyword')
...
model = PETER(args, args.peter_mask, src_len, tgt_len, pad_idx, nuser, nitem, ntokens, args.emsize, args.nhead,
...
def train_img_loss(data, epoch):
    ...
    while True:
        ...
        user, item, rating, seq, feature, item_k, image_feature = data.next_batch()  # (batch_size, seq_len), data.step += 1
        ...
        optimizer.zero_grad()
        log_word_prob, log_context_dis, rating_p, _, image_fea_p = model(user, item, text, image_feature)  # (tgt_len, batch_size, ntoken) vs. (batch_size, ntoken) vs. (batch_size,)
        ...
        if args.image_fea:
            i_loss = img_criterion(image_fea_p, image_feature)
        ...
        if data.step % args.log_interval == 0 or data.step == data.total_step:
            ...
            if args.image_fea:
                cur_i_loss = img_loss / total_sample
                print(now_time() + 'context ppl {:4.4f} | text ppl {:4.4f} | rating loss {:4.4f} | img loss {:4.4f} | {:5d}/{:5d} batches'.format(
                        math.exp(cur_c_loss), math.exp(cur_t_loss), cur_r_loss, cur_i_loss, data.step, data.total_step))
            else:
...
def evaluate_img_loss(data):
    ...
    with torch.no_grad():
        while True:
            ...
            log_word_prob, log_context_dis, rating_p, _, image_fea_p = model(user, item, text,
    ...
    with torch.no_grad():
        while True:
            ...
            for idx in range(args.words):
                ...
                if idx == 0:
                    log_word_prob, log_context_dis, rating_p, _, _ = model(user, item, text, image_feature,False)  # (batch_size, ntoken) vs. (batch_size, ntoken) vs. (batch_size,)
                    ...
                    log_word_prob, _, _, _, _ = model(user, item, text, image_feature, False, False, False)
...
for epoch in range(1, args.epochs + 1):
    ...
    train_img_loss(train_data, epoch - 1)
    val_c_loss, val_t_loss, val_r_loss, img_loss = evaluate_img_loss(val_data)
    ...
    if args.image_fea:
        print(now_time() + 'context ppl {:4.4f} | text ppl {:4.4f} | rating loss {:4.4f} | img loss {:4.4f} | valid loss {:4.4f} on validation'.format(
                math.exp(val_c_loss), math.exp(val_t_loss), val_r_loss, img_loss, val_loss))
    else:
...
test_c_loss, test_t_loss, test_r_loss, img_loss = evaluate_img_loss(test_data)
print('=' * 89)
print(now_time() + 'context ppl {:4.4f} | text ppl {:4.4f} | rating loss {:4.4f} | img loss {:4.4f} on test | End of training'.format(
        math.exp(test_c_loss), math.exp(test_t_loss), test_r_loss, img_loss))


print(now_time() + 'Generating text')
text_o = generate_img_loss(test_data)
