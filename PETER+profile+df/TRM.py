...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask,scale_shift = None):
...
class Encoder(nn.Module):
    def __init__(self, src_emb,d_model,n_layers,n_heads, d_k, d_v,d_ff,dropout, pad_idx, nuser, nitem, args):
        ...
        time_emb_dim = 4* src_emb.embedding_dim
    ...
    def forward(self, user_emb, item_emb, enc_inputs_emb, enc_inputs, T):
        ...
        for layer in self.layers:
            ...
            enc_outputs, enc_self_attn = layer(enc_outputs, enc_self_attn_mask, scale_shift)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask,scale_shift= None ):
        dec_outputs, dec_self_attn = self.dec_self_attn(dec_inputs, dec_inputs, dec_inputs, dec_self_attn_mask)
        if exists(scale_shift):
            scale, shift = scale_shift
            dec_outputs = dec_outputs * (scale + 1) + shift
...
class Decoder(nn.Module):
    def __init__(self,tgt_emb, d_model,n_layers, n_heads, d_k, d_v, d_ff, dropout, pad_idx, peter_mask, args):
        ...
        self.args =args
        self.ui_len = 2
        time_emb_dim = 4* tgt_emb.embedding_dim
        self.mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear(time_emb_dim, d_model*2)
        )

    def forward(self, user_emb, item_emb, dec_inputs_emb, dec_inputs, enc_inputs, enc_outputs,image_feature,T): # dec_inputs : [batch_size x target_len]

        # dec_inputs = dec_inputs.transpose(0, 1)
        # enc_inputs = enc_inputs.transpose(0, 1)

        dec_outputs = dec_inputs_emb# [batch_size, tgt_len, d_model]
        ...
        for layer in self.layers:
            dec_outputs, dec_self_attn, dec_enc_attn = layer(dec_outputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask,scale_shift)
...
class Transformer(nn.Module):
    def __init__(self, args, peter_mask, src_len, tgt_len, src_vocab_size, tgt_vocab_size, d_model, n_layers, n_heads, d_k, d_v, d_ff, dropout, pad_idx, nuser, nitem, alphas_cumprod):
        ...
        # self.step_embeddings = nn.Embedding(args.n_steps,d_model)
        #
    ...
    def extract(self, a, t, x_shape):
        ...
        # print('b{}'.format(b))
        # print('len_x_shape{}'.format(len(x_shape)))

        return out.reshape(b, *((1,) * (len(x_shape) - 1)))

    def add_noise(self,x_0, noise, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t):
        # tmp = self.extract(alphas_bar_sqrt, t, x_0.shape)
        # tmp1 = self.extract(one_minus_alphas_bar_sqrt, t, x_0.shape)
        # print('term1{}term2{}x0{}len_x0{}noise{}'.format(tmp.shape,tmp1.shape,x_0.shape,len(x_0.shape),noise.shape))
        return ( self.extract(alphas_bar_sqrt, t, x_0.shape) * x_0 + self.extract(one_minus_alphas_bar_sqrt, t, x_0.shape) * noise)




    def forward(self, user, item, enc_inputs, dec_inputs, dec_inputs_masked, image_feature, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t, token_flag=False, KD_T = False):
        ...
        # total_len = self.ui_len +dec_inputs.size(0)  # deal with generation when total_len != src_len + tgt_len
        # if self.image_fea:
        #     total_len += 1
        ...
        # enc_emb = self.word_embeddings(enc_inputs) # (len, batch_size, emsize)
        #
        # enc_emb = torch.cat([u_src, i_src, enc_emb], 0)
        # enc_emb = enc_emb * math.sqrt(self.emsize)
        # enc_emb = self.pos_encoder_attention(enc_emb)
        #
        # left = torch.zeros(batch_size, self.ui_len).bool().to(device)  # (batch_size, ui_len)
        # right = dec_inputs.t() == self.pad_idx  # replace pad_idx with True and others with False, (batch_size, total_len - ui_len)
        #
        # words_attn_mask = enc_inputs.t() == self.pad_idx
        # total_words_attn_mask = torch.cat([left, words_attn_mask], 1)
        #
        #
        # key_padding_mask = torch.cat([left, right], 1)  # (batch_size, total_len)
        #
        # # enc_outputs: (len, batch_size, emsize) same as enc_emb
        # enc_outputs, enc_self_attns = self.attention_module(enc_emb, mask= None,src_key_padding_mask = total_words_attn_mask)
        #
        # attn_mask = self.attn_mask[:total_len, :total_len].to(device)  # (total_len, total_len)
        #
        #
        # dec_emb = self.word_embeddings(dec_inputs)  # (total_len - ui_len, batch_size, emsize)
        # dec_emb = torch.cat([u_src, i_src, dec_emb], 0)  # (total_len, batch_size, emsize)
        #
        # dec_emb = dec_emb * math.sqrt(self.emsize)
        ...
        # dec_outputs, dec_self_attns, dec_enc_attns = self.transformer_decoder(dec_emb, enc_outputs, attn_mask, key_padding_mask,total_words_attn_mask)


        user_emb = self.user_embeddings(user).unsqueeze(1)
        item_emb = self.item_embeddings(item).unsqueeze(1)
        enc_inputs_emb = self.word_embeddings(enc_inputs)
        dec_inputs_emb_0 = self.word_embeddings(dec_inputs)

        # t时刻的采样值

        noise = torch.randn_like(dec_inputs_emb_0).to(device)

        dec_inputs_emb_t = self.add_noise(dec_inputs_emb_0, noise, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t)
        dec_inputs_emb_t = dec_inputs_emb_t.float()
        ...
        enc_outputs, enc_self_attns = self.encoder(user_emb, item_emb, enc_inputs_emb, enc_inputs, T)

        # print(enc_self_attns)
        ## dec_outputs 是decoder主要输出，用于后续的linear映射； dec_self_attns类比于enc_self_attns 是查看每个单词对decoder中输入的其余单词的相关性；dec_enc_attns是decoder中每个单词对encoder中每个单词的相关性；
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(user_emb, item_emb, dec_inputs_emb_t, dec_inputs, enc_inputs, enc_outputs,image_feature,T)
        # dec_outputs_masked, dec_self_attns_masked, dec_enc_attns_masked = self.decoder_masked(user_emb, item_emb, dec_inputs_masked, enc_inputs, enc_outputs,image_feature)
        ...
        # rating_masked = self.predict_rating_masked(dec_outputs_masked)
        # log_context_dis_masked = self.predict_context_masked(dec_outputs_masked)
        # if token_flag:
        #     log_word_prob_masked = self.generate_token_masked(dec_outputs_masked)  # (tgt_len, batch_size, ntoken)
        #
        # else:
        #     log_word_prob_masked = self.predict_seq_masked(dec_outputs_masked)  # (tgt_len, batch_size, ntoken)
        # log_side_prob_masked = self.predict_side_masked(dec_outputs_masked)  # (tgt_len, batch_size, ntoken)

        ## dec_outputs做映射到词表大小
        # scentences = self.projection(dec_outputs[:,self.ui_len:,:]) # dec_logits : [batch_size x src_vocab_size x tgt_vocab_size]
        # log_word_prob = func.log_softmax(scentences, dim=-1)

        # torch.save(enc_self_attns,os.path.join(self.weight_path, 'enc_self_attns.pt'))
        # torch.save(dec_self_attns,os.path.join(self.weight_path, 'dec_self_attns.pt'))
        # torch.save(dec_enc_attns,os.path.join(self.weight_path, 'dec_enc_attns.pt'))

        # return log_word_prob, rating, log_context_dis, log_side_prob, dec_outputs, log_word_prob_masked, rating_masked, log_context_dis_masked, log_side_prob_masked, dec_outputs_masked

        # loss = self.loss_fn(dec_outputs[:,2:,:], noise, reduction = 'none')
        # loss = reduce(loss, 'b ... -> b (...)', 'mean')
        # loss = loss * self.extract(self.p2_loss_weight, t, loss.shape)
        # noise_loss = self.loss_fn(dec_outputs[:,2:,:], noise)

        # return loss

        return log_word_prob, rating, log_context_dis
