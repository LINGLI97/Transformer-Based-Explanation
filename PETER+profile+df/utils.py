...
class WordDictionary:
    def __init__(self):
        self.idx2word = ['<bos>', '<eos>', '<pad>', '<unk>','<mask>']
...
class DataLoader:
    def __init__(self, data_path, index_dir, vocab_size, args):
        ...
        self.reviews = self.prep_profile_data_list(smax=self.args.profile_words, dmax=self.args.profile_len, data_path = args.data_path)
    ...
    def load_data(self, args, dir, data_path, index_dir):
        ...
        for review in reviews:
            ...
            # mu, mi = review['user_item_profile_mask'][0], review['user_item_profile_mask'][1]
            # user_profile = self.user_profile[self.user_dict.entity2idx[review['user']]].copy()
            # item_profile = self.item_profile[self.item_dict.entity2idx[review['item']]].copy()
            # if mu < len(user_profile):
            #     user_profile[mu] = [self.__bos]+[self.__eos] + [self.__pad for _ in range(self.args.words)]
            #
            # if mi < len(item_profile):
            #     item_profile[mi] = [self.__bos]+[self.__eos] + [self.__pad for _ in range(self.args.words)]
...
class Batchify:
    def __init__(self, data, word2idx, args, idx2item, seq_len=15, key_len=2, batch_size=128, shuffle=False):
        ...
        pad = word2idx['<pad>']
        mask = word2idx['<mask>']
        u, i, r, t, t_masked, f, a, i_k , u_p, i_p = [], [], [], [],[], [], [], [], [], []
    ...
    def next_batch(self):
        ...
        return user, item, rating, seq, seq_masked, feature, adjective, item_k, image_fea, user_profile, item_profile
